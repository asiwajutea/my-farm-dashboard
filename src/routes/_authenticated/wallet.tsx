...
function TransferToFarmingDialog({
  ...
  const seedAmount = Number.isFinite(usdt) && usdt > 0 ? usdtToSeed(usdt, rate) : 0;
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      toast.success(`Transferred ${fmtAmount(seedAmount)} Seed to farming wallet`);
      setOpen(false);
      setUsdtStr("");
      onDone();
    } catch (e) {
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            Move USDT from your primary wallet. It will be converted to Seed at the current
            rate of 1 Seed = {fmtAmount(rate, 4)} USDT.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-3 py-2">
          <div className="flex items-center justify-between text-xs text-muted-foreground">
            ...
            <span className="tabular-nums">
              {fmtAmount(primaryAvailableUsdt)} USDT · {fmtAmount(primaryAvailableSeed)} Seed
            </span>
          </div>
          <div>
