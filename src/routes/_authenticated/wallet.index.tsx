...
interface WalletRow { kind: WalletKind; balance: number; locked: number; }
interface LedgerRow { id: string; kind: string; amount: number; memo: string | null; created_at: string; }
        ...
        <Loadable loading={loading} skeleton={<ListSkeleton rows={5} leading="none" />}>
          {!ledger || ledger.length === 0 ? (
            ...
            <ul className="divide-y divide-border/40">
              {ledger.map((e) => <LedgerItem key={e.id} entry={e} />)}
            </ul>
          )}
        </Loadable>
      </section>
